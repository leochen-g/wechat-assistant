...
initDay = async() => {
    ...
    let str = `${today}<br>我们在一起的第${memorialDay}天<br><br>元气满满的一天开始啦,要开心噢^_^<br><br>今日天气<br>${weather.weatherTips}<br>${weather.todayWeather}<br>每日一句:<br>${one}<br><br>————————最爱你的我`
    try {
        logMsg = str
    ...
    try {
        for (item of list) {
            ...
            schedule.setSchedule(time, async() => {
                ...
                if (!item.isLoop) {
                    req(host + '/updateSchedule', 'POST', '', { id: item._id }).then((result) => {
...
onLogin = async(user) => {
    ...
    req(host + '/getScheduleList', 'GET').then((res) => {
...
addSchedule = async(obj) => {
    req(host + '/addSchedule', 'POST', '', obj).then(async(res) => {
        ...
        schedule.setSchedule(Rule1, async() => {
            ...
            if (!res.data.isLoop) {
                req(host + '/updateSchedule', 'POST', '', { id: res.data._id }).then((result) => {
                    console.log('更新定时任务成功')
                }).catch(err => {
                    console.log('更新错误', err)
                })
    ...
    try {
        ...
        switch (friendship.type()) {
            ...
            case Friendship.Type.Receive:
                await utils.delay(60000) // 延时1分钟后同意好友
